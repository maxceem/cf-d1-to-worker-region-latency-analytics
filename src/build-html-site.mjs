  ...
  return '<div class="wrap-inner">' +
    ...
    '<div class="intro">' +
      '<div class="intro-text">' +
        '<p class="sub">A D1 query is a round trip, and one request can fire many. A Worker far from its database ' +
          'stacks up latency fast — Cloudflare placement fixes it by pinning the Worker next to the database.</p>' +
        '<p class="idea-cap">But worker locations are named after AWS, GCP, and Azure regions, so the fastest one ' +
          'per D1 region is not obvious. This benchmarks them all to find it.</p>' +
      '</div>' +
...
(function initTheme() {
  ...
  let saved = "dark";
  try { saved = localStorage.getItem("d1theme") || "dark"; } catch (e) {}
